  ...
  bool synchronousLoading, TextureManager::TextureId& textureId, Dali::WrapMode::Type wrapModeU, Dali::WrapMode::Type wrapModeV, TextureUploadObserver* textureObserver )
{
  ...
  else
  {
    ...
    textureId = RequestLoadInternal( animatedImageLoading.GetUrl(), INVALID_TEXTURE_ID, 1.0f, ImageDimensions(), FittingMode::SCALE_TO_FILL,
                                     SamplingMode::BOX_THEN_LINEAR, TextureManager::UseAtlas::NO_ATLAS, false, StorageType::UPLOAD_TO_TEXTURE, textureObserver,
  ...
  const VisualUrl& url, Dali::ImageDimensions desiredSize, Dali::FittingMode::Type fittingMode, Dali::SamplingMode::Type samplingMode, bool synchronousLoading, TextureUploadObserver* textureObserver, bool orientationCorrection, TextureManager::MultiplyOnLoad& preMultiplyOnLoad )
{
  ...
  {
    RequestLoadInternal( url, INVALID_TEXTURE_ID, 1.0f, desiredSize, fittingMode, samplingMode, TextureManager::UseAtlas::NO_ATLAS,
  ...
  TextureManager::ReloadPolicy reloadPolicy, TextureManager::MultiplyOnLoad& preMultiplyOnLoad )
{
  ...
  else
  {
    ...
    if( !textureSet ) // big image, no atlasing or atlasing failed
    {
      ...
      if( !maskInfo || !maskInfo->mAlphaMaskUrl.IsValid() )
      {
        textureId = RequestLoad( url, desiredSize, fittingMode, samplingMode, TextureManager::UseAtlas::NO_ATLAS,
...
TextureManager::TextureId TextureManager::RequestMaskLoad( const VisualUrl& maskUrl )
{
  ...
  return RequestLoadInternal( maskUrl, INVALID_TEXTURE_ID, 1.0f, ImageDimensions(), FittingMode::SCALE_TO_FILL,
                              SamplingMode::NO_FILTER, UseAtlas::NO_ATLAS, false, StorageType::KEEP_PIXEL_BUFFER, NULL, true,
  ...
  uint32_t                        frameIndex )
{
  ...
  int cacheIndex = FindCachedTexture( textureHash, url.GetUrl(), desiredSize, fittingMode, samplingMode, useAtlas == UseAtlas::USE_ATLAS,
      ...
      case LoadState::UPLOADED:
    {
      ...
      else
      {
        ...
        observer->UploadComplete( true, textureInfo.textureId, textureInfo.textureSet,
                                  textureInfo.useAtlas == UseAtlas::USE_ATLAS, textureInfo.atlasRect,
...
{
  if( textureInfo.useAtlas != UseAtlas::USE_ATLAS )
  {
...
void TextureManager::NotifyObservers( TextureInfo& textureInfo, bool success )
{
  ...
  while( info->observerList.Count() )
  {
    ...
    {
      observer->UploadComplete( success, info->textureId, info->textureSet, info->useAtlas == UseAtlas::USE_ATLAS, info->atlasRect,
                                info->preMultiplied );
    }
  ...
  uint32_t                       frameIndex )
{
  ...
  if( width != 0 || height != 0 )
  {
    ...
    *hashTargetPtr   = ( fittingMode << 6u ) | ( samplingMode << 3 ) | ( static_cast<unsigned int>(useAtlas) << 2 ) | static_cast<unsigned int>(storageType);
  }
  else
  {
